...
window.onload = async function() {
    document.getElementById('username_update').disabled = true;
    ...
    fetch('/account/pwcheck', {
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('username_update').disabled = false;
...
async function handleUsernameUpdate(event) {
    ...
    const username_upd = document.getElementById('username_update').value;

    // confirm new updated username
    if (user.user_name === username_upd) {
        alert("Username is not changed. Please try again.");
        return;
    }

    const upd_data = {
        cur_id: user.user_id,
        upd_name: username_upd
    };
